# SINPEND — De/Para (arquivo físico → persistência)

> **Service:** `CoinsuranceSinPendService.groovy`  > **Entidade:** `CoinsuranceRepository.CnsSinPend`

## Origem (arquivo físico)

- Separador CSV: `|` (pipe)
- Leitura: arquivo pendente no S3 → parse em mapa de campos
- **Validação Velora:** `bmg_sinpend`
