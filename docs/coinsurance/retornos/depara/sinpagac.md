# SINPAGAC — De/Para (arquivo físico → persistência)

> **Service:** `CoinsuranceSinpagacService.groovy`  > **Entidade:** `CoinsuranceRepository.CnsSinPagAc`

## Origem (arquivo físico)

- Separador CSV: `|` (pipe)
- Leitura: arquivo pendente no S3 → parse em mapa de campos
- **Validação Velora:** `bmg_sinpagac`
